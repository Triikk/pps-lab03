...
object Streams extends App :
  ...
  object Stream:
    ...
    def takeWhile[A](stream : Stream[A])(pred : A => Boolean) : Stream[A] = (stream,pred) match
      case (Cons(h,t), pred) if pred(h()) => Cons(h,() => takeWhile(t())(pred))
      case _ => Empty()

    def fill[A](k : Int)(elem : A) : Stream[A] = (k,elem) match
